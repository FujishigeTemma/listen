...
import { useTracks, useCreateTrack, useUpdateTrack, useDeleteTrack, useHealth } from '../lib/queries';
...
function TracksPage() {
	...
	const [editingTrack, setEditingTrack] = useState<number | null>(undefined);
...
function TrackRow({
	...
	const [formData, setFormData] = useState({
		...
		timestampSeconds: track.timestamp_seconds,
		artist: track.artist ?? "",
		title: track.title,
		label: track.label ?? "",
	});
		...
		return (
			<div className="space-y-3 rounded-lg border border-zinc-700 bg-zinc-900 p-4">
				<div className="grid grid-cols-4 gap-3">
					...
					<div>
						<label className="text-xs text-zinc-500">Timestamp (sec)</label>
						<input
							type="number"
	...
	return (
		<div
			onClick={onEdit}
			className="flex cursor-pointer items-center gap-4 rounded-lg border border-zinc-800 bg-zinc-900 px-4 py-3 hover:border-zinc-700"
		>
			<span className="w-8 text-zinc-500">{track.position}</span>
			<span className="w-16 text-zinc-500">{formatTimestamp(track.timestamp_seconds)}</span>
